...
ts_gpA_struc(:,1) = ones(length(ts_CC_gpA),1);
ts_gpB_struc(:,1) = ones(length(ts_CC_gpB),1);

% longitudinal
ts_gpAtp2_struc(:,1) = ones(length(ts_CC_gpAtp2),1);
ts_gpBtp2_struc(:,1) = ones(length(ts_CC_gpBtp2),1);
...
ts_struct(:,1) = ones(length(list_node_all),1);
